...
- SVG encoding: distinct frames on a horizontal reel, one discrete CSS animation (`steps(1,end)`), rows interned in `<defs>` shared via `<use>`, and repeated runs inside rows interned a level deeper (`<text id="c…">`/`<rect id="b…">` + position-free `<use>`). Run interning uses exact byte economics (share iff `sum(inline) > def + sum(refs)`, counted over unique rows only) so sparse output is untouched. No JS. Faint uses `fill-opacity` (not `opacity`: `opacity` on `<text>` crashes resvg). Plain text inherits `fill` from the reel `<g>` — never rely on the black default.
